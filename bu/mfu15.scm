...
(define (create-mediafire-sub-folder key path)
  (print "[dir   ] "  path)
  (let-values (((_ dir-name _) (decompose-pathname path)))
    (mediafire-api-call "folder_key"
                        "folder/create.php?"
                        "foldername=" (string-url-encode dir-name)
                        "&action_on_duplicate=replace"
                        "&parent_key=" key)))
...
(define (simple-upload-file file-path file-size)
  (let* ((target-name (pathname-strip-directory file-path)) 
         ...
         (cmd   (string-append 
                   ...
                   "&session_token=" (session) "\""))
         (result (call-with-input-pipe cmd (cut read-string #f <>)))
         (ok     (member (xml-value result "result") '("0" "Success"))))
;   (print "CMD: " cmd)
;   (print "RES: " result)
    (unless ok
      (sleep 1)
    ...
    (let unit-loop ((ix 0))
      (when (< ix nr-of-units)
        ...
        (let* ((dd-part   (string-append 
               ...
               (ok     (member (xml-value result "result") 
                               '("0" "Success"))))
;   (print "CMD: " cmd)
;   (print "RES: " result))
          (if ok
...
(define (upload-directory path #!optional key)
  (if (directory? path)
    (let ((dir-key (if key (create-mediafire-sub-folder key path)
                           (create-mediafire-folder path))))
      (for-each 
        (lambda (name) 
...
(define (main)
; (upload-directory "/home/wiffel/kashbah/mediafire")
; (upload-directory "/home/wiffel/dev/mediafire")
; (upload-directory "/backups")
  (upload-directory "/home/wiffel/Pictures")
  (upload-directory "/home/wiffel/Music")
  (upload-directory "/home/wiffel/Videos")

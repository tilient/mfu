...
import strutils, times, os, streams,
       posix, libcurl, threadpool
#httpclient
...
# discard curl.easy_setopt(OPT_PROXY, proxy);
...
proc upload_file(mediafire_session : MFS, path : string) {.thread.} =
  ...
  try3times(path):
    ...
    elif file_needs_update(path, file_size_str, file_sig, nr_of_units,
                           unit_size, hash_exists, bitmapstr):
      if hash_exists:

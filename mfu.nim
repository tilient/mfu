...
proc collectStr(buffer: cstring; size: int; nitems: int;
                outstream: pointer): int {.cdecl.} =
  let length = nitems * size
  var str = newString(length)
  for ix in 0 .. < length:
    str[ix] = buffer[ix]
  curl_collect_str.add(str)
  length
...
proc string_between(str, start_str, end_str : string): string =
  let
    ...
    strt_pos = pos + start_str.len()
    end_pos  = if (pos >= 0): str.find(end_str, strt_pos) - 1 else: -1
  if end_pos >= 0:
    str.substr(strt_pos, end_pos)
  else:
    "???"
...
proc SHA256ofFileLoop(x:int) =
  ...
  var
    ...
    filenameFileSHA256Cache = expandTilde(params.profiledir /
                                                           "sha_cache")
...
proc simple_upload(path, dir_path, filename: string;
                   file_size: int64) =
  let
...
proc create_mediafire_folder(path,
                             parent_key: string): string {.gcsafe.}
